# Dominican Republic Law MCP Server — Developer Guide
...
Dominican Republic Law MCP server providing Dominican legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB). Covers data protection, cybercrimes, ICT, companies, consumer protection, and other key Acts.
...
1. `scripts/ingest.ts` -> fetches from Dominican Republic Law -> JSON seed files in `data/seed/`
...
- npm package: `@ansvar/dominican republic-law-mcp` with bin entry for stdio
