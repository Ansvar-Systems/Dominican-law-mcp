/**
 * search_eu_implementations — Search EU directives/regulations that have Dominican implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_dominican_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  dominican_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as dominican_statute_count
  ...
  if (input.has_dominican_implementation) {
    sql += ' HAVING dominican_statute_count > 0';
  }
