/**
 * Tool registry for Dominican Republic Law MCP Server.
...
import { getDominicanImplementations, type GetDominicanImplementationsInput } from './get-dominican-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Dominican Republic Law (Consultoría Jurídica del Poder Ejecutivo). ' +
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Dominican statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section/article) from a Dominican statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Dominican legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Section 25, Data Protection Act 2019", "s 25, DPA 2019", "Article 31, Constitution of Dominican Republic 2010".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Dominican statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Dominican Republic?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Dominican legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Dominican statute or provision is currently in force, amended, repealed, partially suspended, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Dominican statute aligns with or references. ' +
      'Dominican Republic is not an EU member but several Dominican laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Dominican statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_dominican_implementations',
    description:
      'Find all Dominican statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Dominican statutes. ' +
      'Note: Dominican Republic implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Dominican implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_dominican_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Dominican aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Dominican statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Dominican statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_dominican_implementations':
          result = await getDominicanImplementations(db, args as unknown as GetDominicanImplementationsInput);
          break;
