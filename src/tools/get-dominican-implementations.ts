/**
 * get_dominican_implementations — Find Dominican statutes implementing a specific EU directive/regulation
...
export interface GetDominicanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface DominicanImplementationResult {
  document_id: string;
...
export async function getDominicanImplementations(
  db: InstanceType<typeof Database>,
  input: GetDominicanImplementationsInput,
): Promise<ToolResponse<DominicanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as DominicanImplementationResult[];
